...
function createFeed(props) {
  ...
  const sortedLatestPosts = [...(latestPosts || [])].sort((a, b) => {
    const dateA = Number(a?.publishDate || a?.lastEditedDate || 0)
    const dateB = Number(b?.publishDate || b?.lastEditedDate || 0)
    return dateB - dateA
  })

  return { feed, latestPosts: sortedLatestPosts, LINK }
}

export async function buildRssXml(props) {
  const { feed, latestPosts, LINK } = createFeed(props)

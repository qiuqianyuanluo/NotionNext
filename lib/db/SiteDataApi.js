...
export async function fetchGlobalAllData({
  ...
  try {
    ...
    // Throwing keeps the last successful ISR page instead of publishing an
    // empty site when Notion is temporarily unavailable.
    throw error
  }

  // 返回给客户端前的清理操作
  const result = handleDataBeforeReturn(deepClone(data))
  if (
    !result?.siteInfo?.title ||
    !Array.isArray(result?.allPages) ||
    result.allPages.length === 0
  ) {
    throw new Error(`Invalid or empty Notion site data: ${pageId}`)
  }
